...
class CalculationService:
    ...
    def get_chart_data(self, chart_id: int, start_date=None, end_date=None) -> Dict[str, Any]:
        try:
        ...
        if config.aggregation_type == 'TIME_HOUR':
             ...
             grouped = df.resample('1h')['value'].agg(['mean', 'std', 'count', 'min', 'max'])
             ...
             stats_series = grouped['mean'] # Calculate global stats on the aggregated means? Or raw? usually Raw for Cpk.

        elif config.aggregation_type == 'COUNT':
             # Group by every N records
             n = config.aggregation_size
             df['group_id'] = np.arange(len(df)) // n
             grouped = df.groupby('group_id')['value'].agg(['mean', 'std', 'count', 'min', 'max'])
             grouped['range'] = grouped['max'] - grouped['min']
